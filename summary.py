...
def plot_values(data: pd.DataFrame, x_var="time", y_var="value", title: str = None, x_label="Time", y_label="Value"):
...
def plot_rolling_mean_together(df: pd.DataFrame, start_year: int, end_year: int):
    df = get_cropped_df(df, start_year, end_year)
    fig, ax1 = plt.subplots(figsize=(12,6))
    ax1.plot(df['month_year'], df['rolling_mean_precipitation'], color="blue", label="Precipitation")
    ...
    ax2 = ax1.twinx()
    ax2.plot(df['month_year'], df['rolling_mean_electricity'], color="green", label="Electricity")
    ax2.set_ylabel('Electricity production(MWh)', color="green")
    ax2.tick_params(axis='y', labelcolor="green")
    if end_year-start_year == 0:
        plt.title(f'Precipitation and electricity production 6-months rolling windows for year {start_year}')
    else:
...
def plot_decomposed(df: pd.DataFrame, period: int):
    decomp = seasonal_decompose(df['value'], model='additive', period=period)
    decomp.plot()
    plt.show()
        ...
        for d in d_values:
            for q in q_values:
                ...
                try:
                    ...
                    continue
    sorted_models = {k: v for k, v in sorted(sorted_models.items(), key=lambda item: item[1])}
...
def plot_insample_preds(preds: pd.DataFrame, ground_truth: pd.DataFrame):
    plt.figure(figsize=(12, 6))
...
def plot_forecast(model_fit, steps: int, ground_truth: pd.DataFrame):
    ...
    plt.figure(figsize=(12,6))
    plt.plot(range(0,len(ground_truth)),ground_truth['value'], label="Ground truth")
    plt.plot(range(len(ground_truth), len(ground_truth)+steps),forecast, color='green', label="forecast")
    plt.legend()
    plt.show()

def plot_forecast_with_exog(forecast: pd.DataFrame, exog: pd.DataFrame):
    fig, ax1 = plt.subplots(figsize=(12,6))
    ax1.set_xlabel("Month")
...
def main():
    ...
    df_weather, df_electricity, df_merged = process_data(WEATHER_DATA_FILEPATH, ELECTRICITY_DATA_FILEPATH)
    df_merged_all_values = df_merged.dropna()

    # PART 1: Exploration
    pre_millenium_cropped_weather_data = get_cropped_df(df_weather, 1998, 1999)
    pre_millenium_cropped_el_data = get_cropped_df(df_electricity, 1998, 1999)
    plot_values(pre_millenium_cropped_weather_data, title="Weather data", y_label="Precipitation(mm)")
    plot_values(pre_millenium_cropped_el_data, title="Electricity production", y_label="Electricity(MWh)")
    ...
    correlation = df_merged['value_precipitation'].corr(df_merged['value_electricity'])
    ...
    plot_monthly_means(df_electricity,"Average electricity production (MWh)", "Average monthly Electricity production")
    plot_monthly_means(df_weather,"Average precipitation(mm)", "Average monthly Precipitation")
    ...
    evaluate_models(df_electricity['value'].values, p_values_grid, d_values_grid, q_values_grid)
    model = ARIMA(df_electricity['value'], **params)
    model_fit = model.fit()
    print(model_fit.summary())
    in_sample_preds = model_fit.predict(start=0, end=len(df_electricity)-1, dynamic=False)
    ...
    forecast_weather_data = get_cropped_df(df_weather, 2012,2013)['value']
    exog_forecast = model_exog_fit.forecast(steps=24, exog=forecast_weather_data)
    plot_forecast_with_exog(exog_forecast, forecast_weather_data)
    armax_insample_preds = model_exog_fit.predict(start=0, end=len(df_electricity)-1, dynamic=False)
